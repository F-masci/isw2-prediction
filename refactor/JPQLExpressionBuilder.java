    ...
    private Object eval(JPQLNode node) {
        switch (node.id) {
            ...
            case JJTBOOLEANLITERAL:
                return factory.newLiteral("true".equalsIgnoreCase(node.text), Literal.TYPE_BOOLEAN);
            ...
            case JJTOR:
                return factory.or(getExpression(left(node)), getExpression(right(node)));

            case JJTAND:
                return and(getExpression(left(node)), getExpression(right(node)));
            ...
            case JJTGREATEROREQUAL:
                return evalComparisonOperator(node);
            ...
            case JJTDIVIDE:
            case JJTMOD:
                return evalArithmeticOperator(node);

            default:
                throw new UnsupportedOperationException("Unsupported node type: " + node.id);
        }
    }

    private Object evalNumericLiteral(JPQLNode node) {
        ...
        BigDecimal num = new BigDecimal(
                (txt.endsWith("l") || txt.endsWith("L") ||
                        ...
                        : txt).multiply(new BigDecimal(negative(node)));

        if (node.id == JJTINTEGERLITERAL)
            return factory.newLiteral(new Long(num.longValue()), Literal.TYPE_NUMBER);
        else
            return factory.newLiteral(num, Literal.TYPE_NUMBER);
    }

    private Object evalComparisonOperator(JPQLNode node) {
        Value val1 = getValue(left(node));
        Value val2 = getValue(right(node));
        setImplicitTypes(val1, val2, (Class<?>) null);

        switch (node.id) {
            case JJTEQUALS: return factory.equal(val1, val2);
            case JJTNOTEQUALS: return factory.notEqual(val1, val2);
            case JJTLESSTHAN: return factory.lessThan(val1, val2);
            case JJTLESSOREQUAL: return factory.lessThanEqual(val1, val2);
            case JJTGREATERTHAN: return factory.greaterThan(val1, val2);
            case JJTGREATEROREQUAL: return factory.greaterThanEqual(val1, val2);
            default: throw new IllegalStateException("Unexpected comparison node: " + node.id);
        }
    }

    private Object evalArithmeticOperator(JPQLNode node) {
        Value val1 = getValue(left(node));
        Value val2 = getValue(right(node));
        setImplicitTypes(val1, val2, Number.class);

        switch (node.id) {
            case JJTADD: return factory.add(val1, val2);
            case JJTSUBTRACT: return factory.subtract(val1, val2);
            case JJTMULTIPLY: return factory.multiply(val1, val2);
            case JJTDIVIDE: return factory.divide(val1, val2);
            case JJTMOD: return factory.mod(val1, val2);
            default: throw new IllegalStateException("Unexpected arithmetic node: " + node.id);
        }
    }
