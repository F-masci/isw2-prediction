...
public class BookieServer implements NIOServerFactory.PacketProcessor, BookkeeperInternalCallbacks.WriteCallback {
    ...
    private void handleAddEntry(ByteBuffer packet, Cnxn src, PacketHeader h, long ledgerId, long entryId, byte[] masterKey, long startTime) {
        int statType = BKStats.STATS_ADD;
        boolean success = false;
        if (bookie.isReadOnly()) {
            LOG.warn("BookieServer is running as readonly mode, so rejecting the request from the client!");
            src.sendResponse(buildResponse(BookieProtocol.EREADONLY, h.getVersion(), h.getOpCode(), ledgerId, entryId));
        } else {
            try {
    ...
    private void handleReadEntry(ByteBuffer packet, Cnxn src, PacketHeader h, long ledgerId, long entryId, long startTime) {
        int statType = BKStats.STATS_READ;
        boolean success = false;
    ...
    public void processPacket(ByteBuffer packet, Cnxn src) {
        ...
        long startTime = isStatsEnabled ? MathUtils.now() : 0;
        long ledgerId = -1;
        ...
        switch (h.getOpCode()) {
            case BookieProtocol.ADDENTRY:
                handleAddEntry(packet, src, h, ledgerId, entryId, masterKey, startTime);
                break;
            case BookieProtocol.READENTRY:
                handleReadEntry(packet, src, h, ledgerId, entryId, startTime);
                break;
            default:
